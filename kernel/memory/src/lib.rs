...
pub fn allocate_frame() -> Option<Frame> {
    FRAME_ALLOCATOR.get().and_then(|fa| fa.lock().allocate_frame())
}

/// Convenience method for allocating several contiguous Frames.
pub fn allocate_frames(num_frames: usize) -> Option<FrameRange> {
    FRAME_ALLOCATOR.get().and_then(|fa| fa.lock().allocate_frames(num_frames))
}
...
-> Result<(Arc<MutexIrqSafe<MemoryManagementInfo>>, Vec<MappedPages>), &'static str> 
{
    ...
    FRAME_ALLOCATOR.get().ok_or("BUG: FRAME_ALLOCATOR not initialized")?.lock().alloc_ready();
